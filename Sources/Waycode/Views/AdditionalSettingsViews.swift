...
struct ShortcutSettingsView: View {
    ...
    private func test(_ shortcut: ShortcutConfiguration) {
        ...
        case .wallpaper: NotificationCenter.default.post(name: .waycodeShowWallpapers, object: nil)
        ...
        case .rightSidebar: NotificationCenter.default.post(name: .waycodeToggleRightSidebar, object: nil)
...
struct GeneralSettingsView: View {
    ...
    var body: some View {
        SettingsGroup("Startup") {
            Toggle("Launch Waycode at login", isOn: Binding(get: { model.configuration.launchAtLogin }, set: { model.setLaunchAtLogin($0) }))
            Text("Login registration works after Waycode is installed as an application bundle.").font(.caption).foregroundStyle(.secondary)
        }
        SettingsGroup("Profiles and saving") {
            ...
            Text("Saved to ~/Library/Application Support/Waycode/config.json. Profiles include the bar, widgets, blur, colors, shortcuts, favorites, tasks, and wallpaper sources.").font(.caption).foregroundStyle(.secondary).textSelection(.enabled)
        }
        SettingsGroup("Desktop integration") {
            ...
            Text("Sebastian II is ported from github.com/sebastianmiletic/hyprland-dotfiles at commit bb7de91. The upstream desktop uses Quickshell, not Waybar, so Waycode maps its layout and palette to native macOS modules.").font(.caption).foregroundStyle(.secondary).textSelection(.enabled)
        }
    }
}
