...
8. The enabled **Wallpaper** add-on sits on the far left of the bar. Clicking it opens a compact themed preview carousel directly below the button. Use the pointer or Left/Right arrows to select, then Return to apply the image to connected displays and Mission Control desktops. Wallpaper folders, image files, and the GitHub archive are managed only in Settings; the archive is detected on disk and is never downloaded again when already installed.
