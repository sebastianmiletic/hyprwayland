...
6. Open **Tiling** to enable Ryft’s built-in Hyprland-style Dwindle engine. One resizable application smoothly fills the safe work area below the Ryft bar; opening a second moves and resizes both windows into halves; each additional application recursively splits and animates the remaining area. Frames stay inside every display and clear of the bar and Dock. No helper app, download, extra menu-bar item, or competing shortcut system is used. Reduce Motion switches placement back to immediate updates.
7. Add a **Shell widget** to display the first output line from any local command on a configurable refresh interval. The battery widget uses 21 visual fill levels in 5% steps, keeps the real charge level while charging, and integrates the bolt into the gauge rather than displaying a falsely full battery. Power-source events update immediately with a one-second fallback refresh.
