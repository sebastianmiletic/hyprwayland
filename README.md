...
9. Open **Keybinds** to create global keyboard actions for Waycode controls, Finder, any selected macOS application, or a shell command. Option+W, Option+A, and Option+N use Carbon hotkeys and remain available behind other apps without an Accessibility-dependent event monitor. Option+1 through Option+9 select the matching Mission Control desktop; clicking a numbered desktop uses the same path. The corresponding Control+number Mission Control shortcuts must be enabled in macOS.
10. Click the Arch mark or press **Option+A** for the Gemini assistant. Press **Option+N** for Controls. The Controls sidebar omits the calendar. Clicking the bar battery toggles Low Power Mode directly without opening a Waycode panel or popover. `pmset` is privileged, so macOS may still show its own administrator authorization sheet. Battery symbols track fullness and use red, orange, yellow, and charging-green state colors.
...
12. Waycode checks notification authorization silently and never opens a permission prompt at launch. Accessibility checks are also silent; only explicit permission controls can open System Settings. Production builds use the stable `Termatica Release Signing` identity when available so macOS can retain TCC grants across updates instead of seeing each build as a different ad-hoc-signed app. macOS permissions cannot be self-granted by an application.
