...
9. Open **Keybinds** to create global keyboard actions for Waycode controls, Finder, any selected macOS application, or a shell command. Option+W, Option+A, and Option+N use Carbon hotkeys backed by global key-state polling and a passive event monitor, with duplicate suppression. None of these fallback paths opens a permission prompt. Option+1 through Option+9 select the matching Mission Control desktop; clicking a numbered desktop uses the same path.
