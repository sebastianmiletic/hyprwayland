...
9. Option+W, Option+A, and Option+N are captured by a session-wide Quartz event tap, independent of the settings window and application focus. Waycode requests macOS Input Monitoring once, retains the decision through its stable signature, and activates the event tap immediately after access is granted. Carbon registration, global key-state polling, and a passive monitor remain fallback paths with duplicate suppression.
10. Click the Arch mark or press **Option+A** for the Gemini assistant. Press **Option+N** for Controls. Clicking the bar battery toggles Low Power Mode directly without opening a Waycode panel or popover. Battery symbols are always white normally or yellow in Low Power Mode—never green. While charging, Waycode uses the integrated `battery.100percent.bolt` symbol so the lightning bolt appears inside the battery.
