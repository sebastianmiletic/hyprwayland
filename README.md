...
10. Wallpaper command shortcuts, including Option+W and Random Wallpaper, are removed from configuration and the keybind editor. Option+A and Option+N remain system-wide sidebar shortcuts. Command+M captures one temporary in-memory frame of the frontmost application’s display and asks Gemini to answer its primary visible question. Multiple-choice results replace the top-left sparkle with the answer letter for three seconds. Short free-form answers expand beside the sparkle for eight seconds, use a notch-safe maximum width, and move neighboring widgets naturally. The frame is sent only after the shortcut is pressed, is never written to disk, and is released after the request.
