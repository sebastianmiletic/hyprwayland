...
9. Open **Keybinds** to create global keyboard actions for Waycode controls, Finder, any selected macOS application, or a shell command. Option+W, Option+A, and Option+N are repaired as guaranteed global defaults at launch and use Carbon hotkeys, with an event-monitor fallback and duplicate-event suppression, so they work while other applications are active. Command+E opens Finder and Command+Q targets the frontmost application.
10. Click the Arch mark or press **Option+A** for the Gemini assistant. Press **Option+N** for Controls. Wi-Fi, sound, battery, Controls, and Settings are separate filled modules in Pills only mode; the grouped source-status cluster is reserved for Sebastian II · 1:1. Battery symbols track 0/25/50/75/100% fullness and use red at 10% or below, orange at 20% or below, yellow for Low Power Mode, and green while connected to power.
...
12. Waycode’s notification daemon checks authorization state before requesting it, so an existing decision is never prompted again. It emits native alerts at 20%, 10%, and 5% battery, suppresses repeats until the Mac has recharged, and keeps recent Waycode alerts in Controls. macOS does not provide a public API for reading Messages or other apps’ notification history; **Open macOS Notification Center** reveals those system-owned alerts instead. Settings and complete bar state save automatically.
