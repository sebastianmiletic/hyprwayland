...
6. Open **Tiling** to enable Ryft’s built-in Hyprland-style Dwindle engine. One visible application is left untouched. Opening a second resizable application smoothly moves and resizes both windows into halves; each additional application recursively splits and animates the remaining area. No helper app, download, extra menu-bar item, or competing shortcut system is used. Reduce Motion switches placement back to immediate updates.
