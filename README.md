...
8. The enabled **Wallpaper** add-on sits on the far left of the bar. Its palette-driven SwiftUI carousel opens directly below the button without a generic title bar or loading spinner. Pointer and Left/Right navigation select a preview; Return applies to the current desktop. Separate actions apply to the current desktop or every Mission Control desktop. Wallpaper sources and the cached GitHub archive are managed only in Settings.
...
10. The Arch assistant mark is rendered as a template and follows the active theme accent. Click it or press **Option+A** for Gemini; press **Option+N** for Controls. Side panels no longer respond to two-finger horizontal swipes and dismiss only after three seconds without mouse, keyboard, or scrolling activity. Clicking the bar battery toggles Low Power Mode directly; charging uses the integrated `battery.100percent.bolt` symbol.
