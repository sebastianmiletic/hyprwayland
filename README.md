...
10. Wallpaper command shortcuts, including Option+W and Random Wallpaper, are removed from configuration and the keybind editor. Option+A and Option+N remain system-wide sidebar shortcuts. Command+M first checks for highlighted text in the frontmost application and sends only that text to Gemini. If nothing is selected, Ryft captures one temporary in-memory image of the focused window and answers its primary visible question. Multiple-choice results replace the top-left sparkle with the answer letter for three seconds. Written answers expand beside the sparkle; long answers scroll across twice and can be clicked to copy. Captured frames are sent only after the shortcut is pressed, are never written to disk, and are released after the request.
